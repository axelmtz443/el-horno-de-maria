...
export default function PedidoPage() {
  ...
  const [form, setForm] = useState<DatosCliente>({
    nombre: "", telefono: "", fecha_entrega: "", hora_entrega: "", notas_generales: "", email: "",
  })
  ...
  function validar() {
    ...
    if (!form.fecha_entrega.trim()) e.fecha_entrega = "Requerido"
    setErrores(e)
    return Object.keys(e).length === 0
  }

  function handleEnviar() {
    if (!validar() || items.length === 0) return
    const link = generarLinkWhatsApp(form, items, total)
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-10">
      <div className="text-center mb-10">
        ...
        <p className="text-[var(--color-pan-600)]">Revisa tu selección y envíalo por WhatsApp</p>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-10">
        ...
        <div className="flex-1 min-w-0">
          ...
          {/* Botones para seguir agregando */}
          <div className="flex flex-wrap gap-3 mb-6 pb-6 border-b border-[var(--color-pan-200)]">
            <Link href="/catalogo"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            {/* Nombre */}
            <div>
              <label className="block text-xs font-semibold text-[var(--color-pan-700)] mb-1">
                Nombre *
              </label>
              <input
                type="text"
                ...
                className={`w-full px-4 py-2.5 rounded-xl border-2 text-sm bg-white outline-none
                  transition-colors focus:border-[var(--color-pan-500)]
                  ${errores.nombre ? "border-red-400" : "border-[var(--color-pan-200)]"}`}
              />
            ...
            <div>
              ...
              <input
                ...
                className={`w-full px-4 py-2.5 rounded-xl border-2 text-sm bg-white outline-none
                  transition-colors focus:border-[var(--color-pan-500)]
                  ${errores.telefono ? "border-red-400" : "border-[var(--color-pan-200)]"}`}
              />
            ...
            <div>
              <label className="block text-xs font-semibold text-[var(--color-pan-700)] mb-1">
                Fecha de entrega *
                <span className="font-normal text-[var(--color-pan-400)] ml-1">(mín. 1 día)</span>
              </label>
              <input
                type="date"
                ...
                className={`w-full px-4 py-2.5 rounded-xl border-2 text-sm bg-white outline-none
                  transition-colors focus:border-[var(--color-pan-500)]
                  ${errores.fecha_entrega ? "border-red-400" : "border-[var(--color-pan-200)]"}`}
              />
              {errores.fecha_entrega && <p className="text-red-500 text-xs mt-1">{errores.fecha_entrega}</p>}
            </div>
            ...
            <div>
              <label className="block text-xs font-semibold text-[var(--color-pan-700)] mb-1">
                Hora preferida
                <span className="font-normal text-[var(--color-pan-400)] ml-1">(opcional)</span>
              </label>
              <input
                type="time"
                value={form.hora_entrega}
                onChange={(e) => setForm({ ...form, hora_entrega: e.target.value })}
                className="w-full px-4 py-2.5 rounded-xl border-2 border-[var(--color-pan-200)] text-sm
                           bg-white outline-none transition-colors focus:border-[var(--color-pan-500)]"
              />
            </div>
            ...
            <div className="sm:col-span-2">
              <label className="block text-xs font-semibold text-[var(--color-pan-700)] mb-1">
                Correo electrónico
                <span className="font-normal text-[var(--color-pan-400)] ml-1">(opcional · para confirmación)</span>
              </label>
              <input
                type="email"
                value={form.email}
            ...
            <div className="sm:col-span-2">
              ...
              <textarea
                ...
                placeholder="Indicaciones especiales, punto de entrega, alergias, etc."
                rows={3}
                className="w-full px-4 py-2.5 rounded-xl border-2 border-[var(--color-pan-200)] text-sm
                           bg-white outline-none transition-colors focus:border-[var(--color-pan-500)] resize-none"
              />
            </div>
          </div>
        </div>
        ...
        <div className="lg:w-72 shrink-0">
          <div className="sticky top-24 bg-white rounded-3xl border border-[var(--color-pan-200)] shadow-sm p-6">
            <h3 className="font-serif text-lg font-semibold text-[var(--color-pan-800)] mb-4">
              Resumen
            </h3>

            <div className="space-y-2 mb-4 max-h-52 overflow-y-auto text-sm">
              {items.map((item) => (
                <div key={item.id} className="flex justify-between text-[var(--color-pan-600)] gap-2">
                  <span className="truncate">{item.cantidad}× {item.producto?.nombre ?? "Pan pers."}</span>
                  <span className="shrink-0 font-medium">${item.precio_unitario * item.cantidad}</span>
                </div>
              ))}
            </div>

            <div className="pt-3 border-t border-[var(--color-pan-100)] flex justify-between
                            font-bold text-[var(--color-pan-900)] mb-6">
            ...
            <p className="text-[var(--color-pan-400)] text-xs text-center mt-3">
              El pago se coordina directamente con la panadería
            </p>
          </div>
        </div>
      </div>
